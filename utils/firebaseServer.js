...
if (!getApps().length) {
  initializeApp({
    ...
    // databaseURL && storageBucket are not included in  the Server key
    // we don't have to include it since we're following the DRY principle
    // what we can do is detructure the CLient env key to get it
...
const profileRef = firestore.collection("profile"); //profileRef

export { bucket, auth, firestore, profileRef };
